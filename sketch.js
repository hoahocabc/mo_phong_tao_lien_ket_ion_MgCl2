...
  sphereLayerButton.mousePressed(toggleSphereLayer);

  labelButton = createButton("Bật nhãn");
  styleButton(labelButton);
  labelButton.mousePressed(toggleLabels);
...
    sphereLayerButton.style("background", "linear-gradient(145deg, #fc5c7d, #6a82fb)");
...
    labelButton.style("background", "linear-gradient(145deg, #fc5c7d, #6a82fb)");
  } else {
...
    }
  } else if (state === "done") {
    // Electron tiếp tục quay
  }

  // Hiển thị lớp cầu nếu bật
  if (showSphereLayer) {
    for (let atom of atoms) {
      push();
...
  // Hiển thị nguyên tử và electron
  for (let atom of atoms) {
    push();
...
  // Xác định Cl nào gần electron nào
